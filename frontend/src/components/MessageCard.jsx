...
export const MessageCard = ({ message, index }) => {
  ...
  const getSentimentIcon = (sentiment) => {
    switch (sentiment) {
      ...
      case 'question': return <HelpCircle size={14} className="text-amber-600" />;
      default: return null; // Neutral doesn't strictly need an icon, or maybe a minus
    }
  };
  ...
  const getSentimentBadge = (sentiment) => {
    ...
    return (
      <span className={twMerge("text-[10px] px-1.5 py-0.5 rounded border flex items-center gap-1 font-medium", styles[sentiment])}>
    ...
    <Draggable draggableId={String(message.id)} index={index}>
      {(provided, snapshot) => (
        ...
        >
          ...
          <p className="text-gray-800 text-sm mb-3 line-clamp-4 leading-relaxed">{message.message}</p>
